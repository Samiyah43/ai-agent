...
describe('runKnowledgeBase', () => {
  ...
  it('returns an error when no query is provided', async () => {
    const result = await runKnowledgeBase('   ', CLIENT_ID, createFakePrisma([]));
  ...
  it('returns an error when the knowledge base is empty', async () => {
    const result = await runKnowledgeBase('refund policy', CLIENT_ID, createFakePrisma([]));
  ...
  it('ranks the most similar chunk first', async () => {
    ...
    const result = await runKnowledgeBase('refund policy', CLIENT_ID, prisma);
